...
- **Stack:** .NET 8 minimal API **+ EF Core** (orchestration & DB) · Python FastAPI (AI: embedding + LLM) · React · **SQL Server 2025** (kiểu VECTOR) · Local AI · Vercel (FE) · Render/VPS (BE).
...
- **CV Suggestion (basic):** Recruiter bấm "Gợi ý CV", hệ thống tìm CV cũ trong Talent Pool có vector gần JD → Top N + điểm. Vector search LUÔN kèm `company_id`. KHÔNG real-time, không gợi ý ngược. (PoC đã chứng minh khả thi.)
...
- **Nhóm hay cá nhân = Recruiter quyết lúc đó, KHÔNG phải 2 mode kỹ thuật:** muốn PV cá nhân → mời 1 ứng viên vào khung; muốn PV nhóm/gom buổi → mời nhiều ứng viên vào CÙNG khung giờ rồi PV lần lượt. Hệ thống chỉ cần "mở khung + magic link + .ics".
...
> **Nhóm hay cá nhân = Recruiter quyết, KHÔNG phải 2 mode kỹ thuật:** PV cá nhân → mời 1 ứng viên vào khung; PV nhóm/gom buổi → mời NHIỀU ứng viên vào cùng khung giờ rồi PV lần lượt. Hệ thống chỉ cần "mở khung + magic link + .ics"; ứng viên hoặc tự chọn trong các khung Recruiter mở, hoặc Recruiter ấn định cứng 1 khung và ứng viên xác nhận có mặt.
...
- **Mỗi ứng viên một bộ khung riêng** (không dùng chung pool → không tranh slot). Recruiter tự canh không gán interviewer trùng giờ. Chống đặt trùng: `UPDATE InterviewSlot SET status='BOOKED' WHERE slot_id=@s AND status='OPEN'` (optimistic locking). *(PV nhóm: Recruiter gán nhiều ứng viên vào cùng khung là chủ ý, không phải lỗi tranh slot.)*
