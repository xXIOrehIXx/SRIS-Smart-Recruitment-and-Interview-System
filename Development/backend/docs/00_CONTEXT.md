...
- **Embedding model (chốt):** `BAAI/bge-m3`, 1024 chiều → `vector(1024)`. Đa ngôn ngữ (hỗ trợ tiếng Việt), đọc tới 8192 token nên embed trọn cả CV 2 trang / CV dài mà không bị cắt cụt. (Bản nhẹ hơn nếu cần sau này: `paraphrase-multilingual-mpnet-base-v2`, 768 chiều.)
...
- Embedding: Python trả `float[]` → .NET bọc `SqlVector<float>` → EF Core lưu `vector(1024)`. Tách Python/.NET KHÔNG đổi.
...
3. **Lưu ý cốt lõi:**
   - DB **SQL Server 2025**; AI **Local AI + Vector** (không OpenAI); embedding `BAAI/bge-m3` (1024 chiều, đọc tới 8192 token).
...
**Việc 2 — Local AI + Vector (23/05):** PoC chạy thật end-to-end "text → embedding → vector → điểm 0–100"; ranking đúng (CV .NET > Java > designer); CV trùng JD ~100đ. *Bài học:* chốt model multilingual (ban đầu MiniLM 384 chiều, sau nâng lên `BAAI/bge-m3` 1024 chiều / 8192 token để đọc trọn CV dài); quy tắc re-embedding; điểm = (1 - cosine_distance)*100; dùng `127.0.0.1` thay `localhost` trên Windows.
