...
const Dashboard = () => {
  ...
  const fetchInterviewerData = async () => {
    try {
      ...
      setPendingGrading(
        normalized.filter((i) => i.status === "PENDING").slice(0, 5),
      );
    } catch (error) {
    ...
    if (isAdmin) {
      return [
        ...
        },
        {
          ...
          value: summary.offerAcceptanceRatePct || 0,
          suffix: "%",
          trend: "+5%",
          trendUp: true,
          icon: <TrophyOutlined />,
      ...
      return [
        {
          ...
          value: summary.recruitmentRequests || 0,
          icon: <FileTextOutlined />,
          color: MATCHA_GREEN,
        },
        {
      ...
      return [
        {
          ...
          value:
            ...
            upcomingInterviews.length + pendingGrading.length,
        ...
        {
          title: "Completed",
          value: dashboardData?.summary?.completed || 0,
          icon: <CheckCircleOutlined />,
          color: "#52c41a",
        },
        {
          title: "Avg Score",
          value: dashboardData?.summary?.avgScore || 82,
          suffix: "%",
          icon: <TeamOutlined />,
          color: "#1890ff",
        },
        ...
        <div className="header-actions">
          ...
          {(isAdmin || isDeptManager) && (
            <Button
              ...
              onClick={() =>
                navigate(
                  isAdmin ? "/recruiter/jobs/create" : "/dept/create-request",
                )
              }
            >
  ...
  const getSubTitle = () => {
    ...
    if (isDeptManager)
      return "Tổng quan yêu cầu tuyển dụng và quyết định hiring";
    if (isInterviewer) return "Welcome back! Here's your interview schedule.";
    return "";
  };

  const pageTitleClass = isAdmin
    ? "admin-dashboard"
    : isDeptManager
      ? "dept-dashboard"
              ...
              <Card className="dashboard-card quick-stat-card" bordered={false}>
                <div className="quick-stat-content">
                  <div
                    ...
                    style={{ backgroundColor: "rgba(93, 140, 62, 0.1)" }}
                  >
                    <CheckCircleOutlined
                      style={{ color: MATCHA_GREEN, fontSize: 24 }}
                    />
                  </div>
                ...
                <Progress
                  percent={
                    dashboardData?.summary?.totalApplications > 0
                      ? Math.round(
                          (dashboardData?.summary?.offersSent /
              ...
              <Card className="dashboard-card quick-stat-card" bordered={false}>
                ...
                <Progress
                  percent={
                    dashboardData?.summary?.offersSent > 0
                      ? Math.round(
                          (dashboardData?.summary?.offersAccepted /
                            dashboardData?.summary?.offersSent) *
                            100,
                        )
                      : 0
              ...
              <Card className="dashboard-card quick-stat-card" bordered={false}>
                <div className="quick-stat-content">
                  ...
                  <div className="quick-stat-info">
                    ...
                    <div className="quick-stat-value">
                      {dashboardData?.summary?.rejected || 0}
                    </div>
                  </div>
                </div>
                <Progress
                      ...
                      render: (_, record) => (
                        <Space>
                          <Button
                            ...
                            size="small"
                            onClick={() =>
                              navigate(`/dept/hiring-decision/${record.id}`)
                            }
                          >
                            Xem
                          </Button>
                ...
                <Table
                  columns={[
                    ...
                    { title: "Vị trí", dataIndex: "position", key: "position" },
                    {
                      ...
                      key: "action",
                      render: (val) => (
                        <Tag color={val === "APPROVED" ? "success" : "error"}>
          ...
          <Row gutter={[20, 20]} style={{ marginTop: 20 }}>
            <Col xs={24} lg={12}>
              <Card className="dashboard-card" bordered={false}>
                ...
                >
                  {(dashboardData?.departmentProgress || []).map(
                    (dept, idx) => (
                      <div key={idx}>
                        <div
                          style={{
                        ...
                        >
                          ...
                          <Text type="secondary">
                            {dept.filled || 0}/{dept.total || 0} vị trí
                          </Text>
                        </div>
                        <Progress
                          percent={
                            dept.total > 0
                              ? Math.round((dept.filled / dept.total) * 100)
                              : 0
                          }
                          strokeColor={dept.color || MATCHA_GREEN}
                        />
                      </div>
                    ),
                  )}
                  ...
                  {(dashboardData?.activities || []).map((item, idx) => (
                    ...
                    >
                      <Avatar
                        size={32}
                        style={{
                          backgroundColor: `${MATCHA_GREEN}15`,
                          color: MATCHA_GREEN,
                          flexShrink: 0,
                          marginTop: 2,
                        }}
            ...
            <Col xs={24} lg={12}>
              <Card className="dashboard-card" bordered={false}>
                <div className="card-header">
                  ...
                  <Button
                    type="link"
                    onClick={() => navigate("/interviewer/grading")}
                  >
                    View All
                  </Button>
                </div>
                    ...
                    <List.Item
                      ...
                      actions={[
                        <Button
                          ...
                          size="small"
                          onClick={() =>
                            navigate(`/interviewer/grading/${item.id}`)
                          }
                        >
                          Grade Now
                        </Button>,
