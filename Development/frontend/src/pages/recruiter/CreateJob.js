...
import {
  ...
  message, Checkbox, Spin, Steps, Tooltip,
  ColorPicker
} from 'antd';
import {
  SaveOutlined,
...
const CreateJob = () => {
    ...
    try {
      ...
      if (job) {
        ...
        form.setFieldsValue({
          ...
          buttonText: job.buttonText || 'Ứng tuyển ngay',
          template: job.template || 'visual',
          backgroundImage: job.backgroundImage || '',
          textColor: job.textColor || '#1f2937'
        });
  ...
  const getFormData = (values) => ({
    ...
    template: values.template || 'visual',
          ...
          <Card className="wizard-sidebar-card" bordered={false}>
            <Steps direction="vertical" current={currentStep} onChange={setCurrentStep}>
              {steps.map((step, index) => (
                <Step key={step.key} title={step.title} icon={index === 0 ? <FileTextOutlined /> : index === 1 ? <SolutionOutlined /> : <BarsOutlined />} />
              ))}
            </Steps>
          </Card>
        </aside>
