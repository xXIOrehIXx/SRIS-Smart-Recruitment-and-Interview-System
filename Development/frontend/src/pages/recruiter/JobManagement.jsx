...
const JobManagement = () => {
  ...
  const getMenuItems = (record) => {
    ...
    return [
      ...
      {
        key: (record.status || '').toLowerCase() === 'open' ? 'pause' : 'resume',
        icon: (record.status || '').toLowerCase() === 'open' ? <PauseCircleOutlined /> : <PlayCircleOutlined />,
        label: (record.status || '').toLowerCase() === 'open' ? 'Tạm Dừng' : 'Kích Hoạt Lại',
      },
      {
        key: 'delete',
        icon: <DeleteOutlined />,
  ...
  const columns = [
      ...
      render: (text, record) => (
        <div className="job-info-cell">
          <div className="job-title-row">
            <span className="job-title">{typeof text === 'string' ? text : String(text ?? '')}</span>
            <Badge
              count={record.applicationCount || (Array.isArray(record.application) ? record.application.length : 0)}
              style={{ backgroundColor: '#5D8C3E' }}
              className="applications-badge"
            />
          </div>
          <div className="job-meta-row">
            <Tag className="dept-tag">{String(record.department ?? record.departmentName ?? 'N/A')}</Tag>
            <Text type="secondary" className="location-text">{String(record.location ?? record.workLocation ?? 'N/A')}</Text>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (salary, record) => {
        ...
        if (typeof salaryText !== 'string' || !salaryText) salaryText = 'Thỏa thuận';
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        const value = typeof raw === 'string' ? raw : (record && typeof record.status === 'string' ? record.status : 'Draft');
        const config = {
