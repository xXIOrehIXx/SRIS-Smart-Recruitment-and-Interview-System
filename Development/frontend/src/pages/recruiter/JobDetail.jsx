...
import {
  Row, Col, Card, Button, Tag, Typography, Descriptions, Tabs, Table, Avatar,
  Progress, Space, Modal, Select, DatePicker, Spin, message, Empty, Tooltip, Divider,
} from 'antd';
import {
  EditOutlined,
  ShareAltOutlined,
...
const JobDetail = () => {
  ...
  const experienceInfo = useMemo(() => {
    const x = typeof job?.experienceLevel === 'string' ? job.experienceLevel : '';
  ...
  const pipelineStats = useMemo(() => {
    ...
    applications.forEach((app) => {
      const st = typeof app.currentState === 'string' ? app.currentState.toUpperCase() : '';
  ...
  const candidatesColumns = [
    {
      ...
      key: 'candidate',
      render: (_, record) => {
        const name = safeString(record.candidateName || record.fullName || record.name);
    ...
    },
    {
      ...
      key: 'state',
      render: (_, record) => {
        const st = typeof record.currentState === 'string' ? record.currentState.toUpperCase() : '';
        const cfg = APPLICATION_STATE_LABEL[st] || { label: st || 'N/A', color: 'default' };
      ...
      render: (_, record) => {
        ...
        return (
          <Tooltip title={`Điểm khớp CV-JD: ${v}/100`}>
            <Progress percent={v} size="small" strokeColor={MATCHA_GREEN} format={(p) => `${p}`} />
          </Tooltip>
        );
      },
    },
      ...
      render: (_, record) => {
        ...
        return (
          <Tooltip title={`Điểm theo tiêu chí: ${v}/100`}>
            <Progress percent={v} size="small" strokeColor="#722ed1" format={(p) => `${p}`} />
          </Tooltip>
        );
      },
    },
        ...
        return (
          <Space>
            <Button
              ...
              onClick={() => {
                if (!appId) {
                  message.warning('Ứng viên này chưa có hồ sơ hợp lệ để xem chi tiết');
                  return;
                }
  ...
  const tabItems = [
    {
      ...
      children: applications.length === 0 && !loading ? (
        <Empty description="Chưa có ứng viên ứng tuyển" />
      ) : (
        <Table
          columns={candidatesColumns}
          dataSource={applications}
          rowKey={(r) => r.applicationId || r.id || `${r.candidateId}-${r.appliedAt}`}
          pagination={{
            pageSize: 10,
          ...
          {pipelineStats.map((item) => (
            <div key={item.key} className="pipeline-stat-item">
              <div className="pipeline-stat-header">
                <span className="stage-dot" style={{ backgroundColor: item.color }} />
      ...
      children: (
        <div className="job-info-content">
          <Descriptions column={1} bordered size="small">
            <Descriptions.Item label={<span><FileTextOutlined /> Mô tả công việc</span>}>
              <Paragraph style={{ whiteSpace: 'pre-line', marginBottom: 0 }}>
                {safeString(job?.jdText || job?.description, 'Chưa cập nhật')}
              </Paragraph>
            </Descriptions.Item>

            <Descriptions.Item label={<span><CheckCircleOutlined /> Yêu cầu</span>}>
              {safeList(job?.requirements).length === 0 ? (
                <Text type="secondary">Chưa cập nhật</Text>
              ) : (
                <ul style={{ paddingLeft: 18, margin: 0 }}>
                  {safeList(job?.requirements).map((req, i) => (
                    <li key={i}>{typeof req === 'string' ? req : safeString(req)}</li>
                  ))}
                </ul>
              )}
            </Descriptions.Item>

            <Descriptions.Item label={<span><GiftOutlined /> Phúc lợi</span>}>
              {safeList(job?.benefits).length === 0 ? (
                <Text type="secondary">Chưa cập nhật</Text>
              ) : (
                <ul style={{ paddingLeft: 18, margin: 0 }}>
                  {safeList(job?.benefits).map((ben, i) => (
                    <li key={i}>{typeof ben === 'string' ? ben : safeString(ben)}</li>
                  ))}
                </ul>
              )}
            </Descriptions.Item>

            <Descriptions.Item label={<span><RocketOutlined /> Kỹ năng</span>}>
              {safeList(job?.skills).length === 0 ? (
                <Text type="secondary">Chưa cập nhật</Text>
              ) : (
                <Space wrap>
                  {safeList(job?.skills).map((s, i) => (
                    <Tag key={i} color="green">{typeof s === 'string' ? s : safeString(s)}</Tag>
                  ))}
                </Space>
              )}
            </Descriptions.Item>
  ...
  if (!job) {
    return (
      <div style={{ padding: 32 }}>
        <Button onClick={() => navigate('/recruiter/jobs')} icon={<ArrowLeftOutlined />}>
          Quay Lại
        </Button>
        <Empty description="Không tìm thấy tin tuyển dụng" style={{ marginTop: 48 }} />
      </div>
    );
  }
          ...
          <Card className="main-card" bordered={false}>
            <div className="job-header">
              <div className="job-info">
                <Title level={3} className="job-title">{safeString(job.title)}</Title>
                <div className="job-tags">
                  <Tag color={statusInfo.color}>{statusInfo.label}</Tag>
                  {employmentInfo && <Tag color={employmentInfo.color}>{employmentInfo.label}</Tag>}
                  {workModeInfo && <Tag color={workModeInfo.color}>{workModeInfo.label}</Tag>}
                  {experienceInfo && <Tag color={experienceInfo.color}>{experienceInfo.label}</Tag>}
                  <Tag icon={<ClockCircleOutlined />}>Đăng ngày {fmtDate(job.createdAt)}</Tag>
                </div>
              </div>
            ...
            <div className="job-details-grid">
              ...
              <div className="detail-item">
                ...
                <div>
                  ...
                  <p>
                    {applications.length}
                    {job.applicationCount > 0 && job.applicationCount !== applications.length
            ...
            {safeList(job.skills).length > 0 && (
              <div className="job-skills-section">
                <Text type="secondary" style={{ marginRight: 8 }}>Kỹ năng:</Text>
                <Space wrap size={[6, 6]}>
                  {safeList(job.skills).map((s, i) => (
                    <Tag key={i} color="green">{typeof s === 'string' ? s : safeString(s)}</Tag>
                  ))}
          ...
          {job.createdBy && (
            <Card className="sidebar-card" bordered={false}>
              ...
              <Descriptions column={1} size="small">
                <Descriptions.Item label="Người tạo">User #{job.createdBy}</Descriptions.Item>
                <Descriptions.Item label="Cập nhật">{fmtDateTime(job.updatedAt)}</Descriptions.Item>
              </Descriptions>
            </Card>
          )}
        </Col>
