...
const TalentPool = () => {
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Avatar
            size={40}
            style={{ backgroundColor: MATCHA_GREEN, flexShrink: 0 }}
            icon={<UserOutlined />}
          />
          <div>
            <div style={{ fontWeight: 600 }}>
              {record.candidateName || record.name || "N/A"}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <MailOutlined style={{ fontSize: 11, color: "#8c8c8b" }} />
              <Text type="secondary" style={{ fontSize: 12 }}>
                {record.email || record.candidateEmail || "N/A"}
              </Text>
            </div>
          </div>
        </div>
      ...
      sorter: (a, b) =>
        (a.candidateName || a.name || "").localeCompare(
          b.candidateName || b.name || "",
        ),
    },
    {
      title: "Kỹ năng",
    ...
    },
    {
      ...
      sorter: (a, b) =>
        (a.matchScore || a.aiScore || 0) - (b.matchScore || b.aiScore || 0),
    },
    {
      ...
      render: (_, record) => {
        const jobTitle =
          record.suggestedJob || record.jobTitle || record.job?.title;
  ...
  const filteredData = talents.filter((talent) => {
    ...
    return (
      (talent.candidateName || talent.name || "")
        .toLowerCase()
        .includes(searchLower) ||
      (talent.email || talent.candidateEmail || "")
        .toLowerCase()
        .includes(searchLower) ||
  ...
  const stats = [
    ...
    {
      title: "Sẵn sàng",
      value: talents.filter((t) => (t.poolStatus || t.status) === "AVAILABLE")
        .length,
      color: "#52c41a",
    },
    {
      title: "Đã mời",
      value: talents.filter((t) => (t.poolStatus || t.status) === "INVITED")
        .length,
      color: "#faad14",
    },
    {
      title: "Tuyển thành công",
      value: talents.filter((t) => (t.poolStatus || t.status) === "HIRED")
        .length,
      color: MATCHA_GREEN,
    },
  ];
    ...
    <div className="talent-pool-page">
      <div className="page-header">
        <div>
          ...
          <Text type="secondary">
            Gợi ý ứng viên tiềm năng từ các ứng viên đã ứng tuyển trước đó
          </Text>
        </div>
        <Button
          icon={<ReloadOutlined />}
        ...
        {selectedTalent && (
          <div style={{ marginTop: 20 }}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <Avatar
                size={72}
                style={{ backgroundColor: MATCHA_GREEN, marginBottom: 12 }}
                icon={<UserOutlined />}
              />
              <Title level={4} style={{ margin: 0 }}>
                {selectedTalent.candidateName || selectedTalent.name}
              </Title>
              <Text type="secondary">
                {selectedTalent.email || selectedTalent.candidateEmail}
              </Text>
            ...
            <Descriptions column={1} size="small">
              ...
              {selectedTalent.matchScore && (
                <Descriptions.Item label="Điểm phù hợp">
                  <Tag
                    color={
                      getMatchScoreColor(selectedTalent.matchScore) ===
                      "#52c41a"
                        ? "success"
                        : getMatchScoreColor(selectedTalent.matchScore) ===
                            "#faad14"
        ...
        {selectedTalent && (
          ...
          >
            ...
            <Form.Item label="Lời nhắn (tùy chọn)" name="message">
              <Input.TextArea
                rows={3}
                placeholder="Viết lời mời gửi đến ứng viên..."
              />
            </Form.Item>
