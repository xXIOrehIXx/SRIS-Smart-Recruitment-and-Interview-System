...
const OfferManagement = () => {
  ...
  const fetchApplications = async (jobId = selectedJobId) => {
    ...
    try {
      ...
      const selectedJob =
        jobs.find((job) => (job.jobId || job.id) === jobId) || null;
      ...
      const offerableApps = apps
        .map((app) => ({
          ...
          job: {
            ...
            title:
              selectedJob?.title || app.job?.title || app.jobTitle || "N/A",
          },
          applicationStatus: app.currentState || app.status,
          appliedAt: app.appliedAt || app.createdAt,
        }))
        .filter((app) =>
          ["NEW", "SCREENING", "INTERVIEW", "INTERVIEWING", "OFFER"].includes(
            app.status,
          ),
        );
  ...
  const fetchOffers = async (jobId = selectedJobId) => {
    ...
    try {
      ...
      const offerPromises = apps
        .filter(
          (app) =>
            app.offer || app.status === "OFFER" || app.currentState === "OFFER",
        )
        .map(async (app) => {
          try {
            const offerRes = await offerAPI.getByApplication(
  ...
  const handleCreateOffer = async (values) => {
    try {
      ...
      await offerAPI.create(
        selectedApplication.id || selectedApplication.applicationId,
        payload,
      );
  ...
  const openCreateModal = (application) => {
    ...
    form.setFieldsValue({
      candidate:
        application.candidateName || application.candidate?.fullName || "N/A",
  ...
  const getAppStatusTag = (status) => {
    ...
    return (
      <Tag color={colors[normalizedStatus] || "default"}>{status || "N/A"}</Tag>
    );
  };
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <Avatar
            style={{ backgroundColor: "#5D8C3E", flexShrink: 0 }}
            icon={<UserOutlined />}
          />
          <div>
  ...
  const selectedJob =
    jobs.find((job) => (job.jobId || job.id) === selectedJobId) || null;

  const tableData = applications.map((app) => {
    const matchedOffer = offers.find(
      (offer) =>
        (offer.applicationId || offer.id) === (app.id || app.applicationId),
    );
    return {
      ...app,
      ...(matchedOffer || {}),
        ...
        title={
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              ...
              <Text type="secondary" style={{ fontSize: 12 }}>
                Gửi offer cho ứng viên đã phỏng vấn
              </Text>
            </div>
          </div>
        }
      ...
      >
        {applications.length === 0 && !selectedApplication ? (
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            <Text type="secondary">
              Không có ứng viên nào đủ điều kiện để tạo offer.
            </Text>
            <br />
            <Text type="secondary">
              Ứng viên cần hoàn thành phỏng vấn trước khi nhận offer.
            </Text>
          </div>
        ) : (
          <Form
          ...
          >
            ...
            <Form.Item
              ...
              name="deadline"
              rules={[
                { required: true, message: "Vui lòng chọn hạn phản hồi" },
              ]}
            >
              <DatePicker
                style={{ width: "100%" }}
            ...
            <Form.Item label="Ghi chú cho ứng viên" name="notes">
              <TextArea
                rows={3}
          ...
          <div>
            ...
            <Descriptions column={1} size="small" style={{ marginBottom: 16 }}>
              ...
              <Descriptions.Item label="Mức lương">
                <Text strong style={{ color: "#5D8C3E" }}>
                  {selectedOffer.salary
                    ? `${Number(selectedOffer.salary).toLocaleString("vi-VN")} VNĐ`
                    : "Thỏa thuận"}
                </Text>
              </Descriptions.Item>
              <Descriptions.Item label="Ngày bắt đầu">
                {selectedOffer.startDate
                  ? dayjs(selectedOffer.startDate).format("DD/MM/YYYY")
                  : "N/A"}
              </Descriptions.Item>
              <Descriptions.Item label="Hạn phản hồi">
                {selectedOffer.deadline ? (
                  <Text
                    type={
                      dayjs(selectedOffer.deadline).isBefore(dayjs(), "day")
                        ? "danger"
                        : undefined
                    }
                  >
            ...
            {selectedOffer.candidateResponse && (
              <>
                ...
                <Tag
                  color={
                    selectedOffer.candidateResponse === "ACCEPTED"
                      ? "success"
                      : "error"
                  }
                >
