...
export const authAPI = {
  ...
  refreshToken: () => api.post("/account/refresh-token"),
...
export const jobsAPI = {
  ...
  getPublicJobs: () => publicApi.get("/jobs"),

  getPublicJob: (jobId) => publicApi.get(`/jobs/${jobId}`),

  // Public: ứng tuyển job
  applyForJob: (jobId, data) => publicApi.post(`/jobs/${jobId}/apply`, data),

  // Public career site với slug
  getPublicJobsBySlug: (slug) => publicApi.get(`/public/${slug}/jobs`),
...
export const cvScoringAPI = {
  ...
  getRanking: (jobId) => api.get(`/cv-scoring/jobs/${jobId}/ranking`),

  getCVScore: (cvId) => api.get(`/cv-scoring/cv/${cvId}?file-url=1`),

  getFileUrl: (cvId) => api.get(`/cv-scoring/cv/${cvId}/file-url`),
};
...
export const applicationAPI = {
  ...
  createMagicLink: (id, purpose, expiresIn = 48) =>
    api.post(
      `/applications/${id}/magic-links?purpose=${purpose}&expiresInHours=${expiresIn}`,
    ),
};
...
export const interviewAPI = {
  ...
  getAllSchedules: (jobId) =>
    api.get(
      jobId ? `/interview-schedules?jobId=${jobId}` : "/interview-schedules",
    ),

  getInterviewPools: (jobId) => api.get(`/jobs/${jobId}/interview-pools`),

  // Alias rõ ràng hơn — Pool theo job (Section 15 flow)
  getPoolsByJob: (jobId) => api.get(`/jobs/${jobId}/interview-pools`),

  createPool: (jobId, data) => api.post(`/jobs/${jobId}/interview-pools`, data),
  ...
  getInterviewers: () => api.get("/interview-pools/interviewers"),
  ...
  reschedule: (applicationId, scheduleId, newSlotId) =>
    api.put(
      `/applications/${applicationId}/interview-schedules/${scheduleId}/reschedule`,
      { slotId: newSlotId },
    ),

  cancelSchedule: (applicationId, scheduleId, reason) =>
    api.post(
      `/applications/${applicationId}/interview-schedules/${scheduleId}/cancel`,
      { reason },
    ),

  // Interviewer's schedules
  getMySchedules: () => api.get("/me/interview-schedules"),
...
export const candidateAPI = {
  getStatus: (token) => api.get(`/candidate/status?token=${token}`),

  getSchedule: (token) => api.get(`/candidate/schedule?token=${token}`),
  ...
  getOffer: (token) => api.get(`/candidate/offer?token=${token}`),
...
export const offerAPI = {
  ...
  withdraw: (offerId) => api.post(`/offers/${offerId}/withdraw`),
};
...
export const mailTemplateAPI = {
  ...
  getById: (templateId) => api.get(`/email-templates/${templateId}`),

  create: (data) => api.post("/email-templates", data),

  update: (templateId, data) => api.put(`/email-templates/${templateId}`, data),
...
export const dashboardAPI = {
  ...
  getFunnelData: (jobId) => api.get(`/dashboard/funnel?jobId=${jobId}`),

  getSourceAnalytics: () => api.get("/dashboard/sources"),
};

// ==================== TALENT POOL ====================

export const talentPoolAPI = {
  getSuggestions: (jobId) => api.get(`/jobs/${jobId}/talent-pool`),

  getAll: () => api.get("/talent-pool"),
};
