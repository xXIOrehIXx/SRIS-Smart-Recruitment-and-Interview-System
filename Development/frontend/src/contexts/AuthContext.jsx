...
export const ROLE_MENUS = {
  ...
  [ROLES.DEPARTMENT_MANAGER]: [
    ...
    {
      key: "/dept/hiring-decision",
      icon: "AuditOutlined",
      label: "Quyết Định Tuyển Dụng",
    },
    {
      key: "/dept/create-request",
...
export const hasPermission = (userRole, route) => {
  ...
  const rolePermissions = {
    ...
    [ROLES.INTERVIEWER]: ["/interviewer", "/settings"],
    [ROLES.DEPARTMENT_MANAGER]: ["/dept", "/settings"],
  };
  ...
  const parseJwt = (token) => {
    try {
      ...
      if (!role) {
        ...
        const roleKey = Object.keys(payload).find(
          (k) =>
      ...
      let email =
        payload.Email || payload.email || payload.Username || payload.username;

      // Tìm fullName - thử nhiều key
      let fullName =
        payload.FullName || payload.fullName || payload.Name || payload.name;
  ...
  const register = async (data) => {
    try {
      ...
      localStorage.setItem(
        "user",
        JSON.stringify(responseData.user || responseData),
      );
